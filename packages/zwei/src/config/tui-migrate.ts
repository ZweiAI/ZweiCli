...
async function opencodeFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.ZWEI_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("zwei", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "zwei")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "zwei"))
  }
  if (Flag.ZWEI_CONFIG) files.push(Flag.ZWEI_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "zwei"))
