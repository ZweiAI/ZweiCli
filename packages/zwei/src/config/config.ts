...
export namespace Config {
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/zwei"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "zwei")
      default:
        return "/etc/zwei"
    }
  }
  ...
  export function managedConfigDir() {
    return process.env.ZWEI_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
  }
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.zwei/command/", "/.zwei/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.zwei/agent/", "/.zwei/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
      ...
      Effect.gen(function* () {
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          const scope = Effect.fnUntraced(function* (source: string) {
            ...
            if (source === "ZWEI_CONFIG_CONTENT") return "local"
          ...
          if (Flag.ZWEI_CONFIG) {
            yield* merge(Flag.ZWEI_CONFIG, yield* loadFile(Flag.ZWEI_CONFIG))
            log.debug("loaded custom config", { path: Flag.ZWEI_CONFIG })
          }

          if (!Flag.ZWEI_DISABLE_PROJECT_CONFIG) {
            for (const file of yield* Effect.promise(() =>
              ConfigPaths.projectFiles("zwei", ctx.directory, ctx.worktree),
            )) {
          ...
          if (Flag.ZWEI_CONFIG_DIR) {
            log.debug("loading config from ZWEI_CONFIG_DIR", { path: Flag.ZWEI_CONFIG_DIR })
          }

          const deps: Promise<void>[] = []

          for (const dir of unique(directories)) {
            if (dir.endsWith(".zwei") || dir === Flag.ZWEI_CONFIG_DIR) {
          ...
          if (process.env.ZWEI_CONFIG_CONTENT) {
            const source = "ZWEI_CONFIG_CONTENT"
            const next = yield* loadConfig(process.env.ZWEI_CONFIG_CONTENT, {
              dir: ctx.directory,
              source,
            })
            yield* merge(source, next, "local")
            log.debug("loaded custom config from ZWEI_CONFIG_CONTENT")
          }
          ...
          if (activeOrg) {
            yield* Effect.gen(function* () {
              ...
              if (Option.isSome(tokenOpt)) {
                process.env["ZWEI_CONSOLE_TOKEN"] = tokenOpt.value
                Env.set("ZWEI_CONSOLE_TOKEN", tokenOpt.value)
              }
          ...
          if (Flag.ZWEI_PERMISSION) {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.ZWEI_PERMISSION))
          }
          ...
          if (Flag.ZWEI_DISABLE_AUTOCOMPACT) {
            result.compaction = { ...result.compaction, auto: false }
          }
          if (Flag.ZWEI_DISABLE_PRUNE) {
