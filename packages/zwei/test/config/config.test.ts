...
const managedConfigDir = process.env.ZWEI_TEST_MANAGED_CONFIG_DIR!
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["ZWEI_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:ZWEI_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["ZWEI_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["ZWEI_CONSOLE_TOKEN"]
    }
  }
})
...
test("loads config from .opencode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".zwei")
...
test("loads agents from .zwei/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".zwei")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .zwei/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".zwei")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .zwei/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".zwei")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("does not try to install dependencies in read-only ZWEI_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.ZWEI_CONFIG_DIR
  process.env.ZWEI_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.ZWEI_CONFIG_DIR
    else process.env.ZWEI_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable ZWEI_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
  ...
  const install = spyOn(Npm, "install").mockImplementation(async (dir: string) => {
    ...
    await Filesystem.write(
      ...
      JSON.stringify({ name: "@zwei/plugin", version: "1.0.0" }),
    )
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const opencodeDir = path.join(projectDir, ".zwei")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".zwei")
...
// Note: preload.ts sets ZWEI_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.zwei/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.zwei/plugin/demo.ts", "file:///project/.zwei/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.zwei/plugin/demo.ts"])
  })
...
describe("ZWEI_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["ZWEI_DISABLE_PROJECT_CONFIG"]
    process.env["ZWEI_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["ZWEI_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["ZWEI_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .zwei/ directories when flag is set", async () => {
    const originalEnv = process.env["ZWEI_DISABLE_PROJECT_CONFIG"]
    process.env["ZWEI_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          const opencodeDir = path.join(dir, ".zwei", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["ZWEI_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ZWEI_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["ZWEI_CONFIG_DIR"]
      process.env["ZWEI_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["ZWEI_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["ZWEI_CONFIG_DIR"]
      } else {
        process.env["ZWEI_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("ZWEI_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["ZWEI_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ZWEI_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["ZWEI_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["ZWEI_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from ZWEI_CONFIG_DIR, not project
...
describe("ZWEI_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in ZWEI_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ZWEI_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["ZWEI_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["ZWEI_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["ZWEI_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in ZWEI_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ZWEI_CONFIG_CONTENT"]
