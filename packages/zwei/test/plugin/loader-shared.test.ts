...
const disableDefault = process.env.ZWEI_DISABLE_DEFAULT_PLUGINS
process.env.ZWEI_DISABLE_DEFAULT_PLUGINS = "1"
...
afterAll(() => {
  if (disableDefault === undefined) {
    delete process.env.ZWEI_DISABLE_DEFAULT_PLUGINS
    return
  }
  process.env.ZWEI_DISABLE_DEFAULT_PLUGINS = disableDefault
})
...
`,
  ...
  test("skips external plugins in pure mode", async () => {
    ...
    const pure = process.env.ZWEI_PURE
    ...
    try {
      ...
      if (pure === undefined) {
        delete process.env.ZWEI_PURE
      } else {
        process.env.ZWEI_PURE = pure
      }
    }
  })
