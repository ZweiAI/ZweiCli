...
// Bundled provider SDKs are loaded lazily on first use. Keeping them off the
// top-level import graph saves ~17 native modules from being eagerly evaluated
// during cold start (TUI startup time). Only types are needed up front.
import type { AmazonBedrockProviderSettings } from "@ai-sdk/amazon-bedrock"
import type { createGitLab } from "gitlab-ai-provider"
...
export namespace Provider {
  ...
  function e2eURL() {
    const url = Env.get("ZWEI_E2E_LLM_URL")
  ...
  type BundledFactory = (options: any) => BundledSDK
  // Each entry is `() => Promise<factory>` — the SDK module isn't pulled into
  // the bundle until `resolveSDK` actually needs it. Cache the resolved factory
  // in `BUNDLED_CACHE` so subsequent loads are zero-cost.
  const BUNDLED_LOADERS: Record<string, () => Promise<BundledFactory>> = {
    "@ai-sdk/amazon-bedrock": () =>
      import("@ai-sdk/amazon-bedrock").then((m) => m.createAmazonBedrock as unknown as BundledFactory),
    "@ai-sdk/anthropic": () =>
      import("@ai-sdk/anthropic").then((m) => m.createAnthropic as unknown as BundledFactory),
    "@ai-sdk/azure": () => import("@ai-sdk/azure").then((m) => m.createAzure as unknown as BundledFactory),
    "@ai-sdk/google": () =>
      import("@ai-sdk/google").then((m) => m.createGoogleGenerativeAI as unknown as BundledFactory),
    "@ai-sdk/google-vertex": () =>
      import("@ai-sdk/google-vertex").then((m) => m.createVertex as unknown as BundledFactory),
    "@ai-sdk/google-vertex/anthropic": () =>
      import("@ai-sdk/google-vertex/anthropic").then((m) => m.createVertexAnthropic as unknown as BundledFactory),
    "@ai-sdk/openai": () => import("@ai-sdk/openai").then((m) => m.createOpenAI as unknown as BundledFactory),
    "@ai-sdk/openai-compatible": () =>
      import("@ai-sdk/openai-compatible").then((m) => m.createOpenAICompatible as unknown as BundledFactory),
    "@openrouter/ai-sdk-provider": () =>
      import("@openrouter/ai-sdk-provider").then((m) => m.createOpenRouter as unknown as BundledFactory),
    "@ai-sdk/xai": () => import("@ai-sdk/xai").then((m) => m.createXai as unknown as BundledFactory),
    "@ai-sdk/mistral": () => import("@ai-sdk/mistral").then((m) => m.createMistral as unknown as BundledFactory),
    "@ai-sdk/groq": () => import("@ai-sdk/groq").then((m) => m.createGroq as unknown as BundledFactory),
    "@ai-sdk/deepinfra": () => import("@ai-sdk/deepinfra").then((m) => m.createDeepInfra as unknown as BundledFactory),
    "@ai-sdk/cerebras": () => import("@ai-sdk/cerebras").then((m) => m.createCerebras as unknown as BundledFactory),
    "@ai-sdk/cohere": () => import("@ai-sdk/cohere").then((m) => m.createCohere as unknown as BundledFactory),
    "@ai-sdk/gateway": () => import("@ai-sdk/gateway").then((m) => m.createGateway as unknown as BundledFactory),
    "@ai-sdk/togetherai": () =>
      import("@ai-sdk/togetherai").then((m) => m.createTogetherAI as unknown as BundledFactory),
    "@ai-sdk/perplexity": () =>
      import("@ai-sdk/perplexity").then((m) => m.createPerplexity as unknown as BundledFactory),
    "@ai-sdk/vercel": () => import("@ai-sdk/vercel").then((m) => m.createVercel as unknown as BundledFactory),
    "gitlab-ai-provider": () =>
      import("gitlab-ai-provider").then((m) => m.createGitLab as unknown as BundledFactory),
    "@ai-sdk/github-copilot": () =>
      import("./sdk/copilot").then((m) => m.createOpenaiCompatible as unknown as BundledFactory),
    "venice-ai-sdk-provider": () =>
      import("venice-ai-sdk-provider").then((m) => m.createVenice as unknown as BundledFactory),
  }
  const BUNDLED_CACHE = new Map<string, BundledFactory>()
  async function loadBundled(npm: string): Promise<BundledFactory | undefined> {
    const cached = BUNDLED_CACHE.get(npm)
    if (cached) return cached
    const loader = BUNDLED_LOADERS[npm]
    if (!loader) return undefined
    const fn = await loader()
    BUNDLED_CACHE.set(npm, fn)
    return fn
  }
  ...
  function custom(dep: CustomDep): Record<string, CustomLoader> {
    return {
      ...
      gitlab: Effect.fnUntraced(function* (input: Info) {
        ...
        // Lazy-load gitlab-ai-provider so non-GitLab users don't pay its
        // import cost at startup.
        const gitlabSDK = yield* Effect.promise(() => import("gitlab-ai-provider"))
        const GITLAB_PROVIDER_VERSION = gitlabSDK.VERSION
        const isWorkflowModel = gitlabSDK.isWorkflowModel
        const discoverWorkflowModels = gitlabSDK.discoverWorkflowModels
    ...
    Effect.gen(function* () {
        ...
        Effect.gen(function* () {
          ...
          for (const [id, provider] of Object.entries(providers)) {
            ...
            for (const [modelID, model] of Object.entries(provider.models)) {
              ...
              if (model.status === "alpha" && !Flag.ZWEI_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
      ...
      async function resolveSDK(model: Model, s: State) {
        try {
          ...
          const bundledFn = await loadBundled(model.api.npm)
          if (bundledFn) {
