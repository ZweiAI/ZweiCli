...
declare const ZWEI_LIBC: string | undefined
  ...
  const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
    try {
      const binding = require(
        `@parcel/watcher-${process.platform}-${process.arch}${process.platform === "linux" ? `-${ZWEI_LIBC || "glibc"}` : ""}`,
      )
    ...
    Effect.gen(function* () {
      ...
      const state = yield* InstanceState.make(
        Effect.fn("FileWatcher.state")(
          function* () {
            if (yield* Flag.ZWEI_EXPERIMENTAL_DISABLE_FILEWATCHER) return
            ...
            if (yield* Flag.ZWEI_EXPERIMENTAL_FILEWATCHER) {
