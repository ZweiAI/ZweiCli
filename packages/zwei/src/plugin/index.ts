import type { Hooks, PluginInput, Plugin as PluginInstance, PluginModule } from "@zwei/plugin"
import { Config } from "../config/config"
import { Bus } from "../bus"
import { Log } from "../util/log"
import { createOpencodeClient } from "@zwei/sdk"
      ...
      const state = yield* InstanceState.make<State>(
        Effect.fn("Plugin.state")(function* (ctx) {
          ...
          const client = createOpencodeClient({
            ...
            headers: Flag.ZWEI_SERVER_PASSWORD
              ? {
                  Authorization: `Basic ${Buffer.from(`${Flag.ZWEI_SERVER_USERNAME ?? "zwei"}:${Flag.ZWEI_SERVER_PASSWORD}`).toString("base64")}`,
                }
              : undefined,
          ...
          const plugins = Flag.ZWEI_PURE ? [] : (cfg.plugin_origins ?? [])
          if (Flag.ZWEI_PURE && cfg.plugin_origins?.length) {
