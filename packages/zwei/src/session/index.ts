import { Slug } from "@zwei/util/slug"
...
export namespace Session {
  ...
  export function plan(input: { slug: string; time: { created: number } }) {
    const base = Instance.project.vcs
      ? path.join(Instance.worktree, ".zwei", "plans")
  ...
  export interface Interface {
    ...
    /**
     * Delete every message (and its parts) for a session while keeping the session row
     * itself. Returns the number of messages removed. Primary consumer: the dual-agent
     * TUI's `/clear1` / `/clear2` slash commands — a run keeps the same child session
     * ids across rounds, so we can't `remove()` them wholesale.
     */
    readonly clear: (id: SessionID) => Effect.Effect<number>
  ...
  export const layer: Layer.Layer<Service, never, Bus.Service | Storage.Service> = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const createNext = Effect.fn("Session.createNext")(function* (input: {
        ...
        if (!Flag.ZWEI_EXPERIMENTAL_WORKSPACES) {
      ...
      const clear = Effect.fn("Session.clear")(function* (id: SessionID) {
        // Collect ids first — `removeMessage` fires SyncEvent.Removed which mutates the
        // underlying store, so we must not iterate the live stream while deleting.
        const ids: MessageID[] = []
        for (const m of MessageV2.stream(id)) ids.push(m.info.id)
        for (const mid of ids) yield* removeMessage({ sessionID: id, messageID: mid })
        return ids.length
      })
  ...
  export const clear = fn(SessionID.zod, (id) => runPromise((svc) => svc.clear(id)))
