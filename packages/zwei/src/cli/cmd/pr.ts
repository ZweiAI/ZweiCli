  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                UI.println(`Found zwei session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["zwei", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting zwei...")
        UI.println()

        const zweiArgs = sessionId ? ["-s", sessionId] : []
        const zweiProcess = Process.spawn(["zwei", ...zweiArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await zweiProcess.exited
        if (code !== 0) throw new Error(`zwei exited with code ${code}`)
      },
    })
  },
})
