...
Replace runtime Bun shell template-tag usage in `packages/zwei/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/zwei/src` is removed except:
