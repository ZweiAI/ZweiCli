...
<p align="right"><a href="./README.md">English</a> | <b>简体中文</b> | <a href="./README.ja.md">日本語</a> | <a href="./README.de.md">Deutsch</a></p>
...
Zwei 的核心理念很简单：把一个编码 agent 拆成**两个隔离的角色** —— PhD 写代码，Supervisor 审代码 —— 信息**单向流动**，写代码的看不到审代码的推理过程。

它针对的是一个真实存在的失败模式：**单 agent 在长对话中的自我评估越来越不靠谱**（所谓的 "Goodharting" —— 自己给自己的作业打分，还总是全 A）。

举个具体的例子。让一个 agent 写一个排序函数并自测：

- **单 agent 模式下：** 它可能写了个有 bug 的测试，然后"通过"了自己这个有 bug 的测试，还宣称大功告成。
- **Zwei 的双 agent 模式下：** PhD 写代码，但**看不到测试、也没有 `bash` 权限**。Supervisor 独立审查代码并运行测试。两边的上下文互不污染。

这是一个**架构押注**：_角色隔离 + 信息不对称 > 单一上下文做所有事_。这个方向值得探索，尤其是在复杂、多步骤的编码任务中。但目前 Zwei 更像是**一个有前途的实验，而不是一个已经被验证过的工具**。如果你对 agent 架构设计感兴趣，值得看看；如果只是想找一个日常使用的高效编码助手，可以过段时间再回来。
