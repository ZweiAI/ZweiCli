...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@zwei/sdk/v2"
...
export async function handleRun(args: RunArgs) {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.ZWEI_AUTO_SHARE && !args.share) return
    ...
    if (args.attach) {
      const headers = (() => {
        const password = args.password ?? process.env.ZWEI_SERVER_PASSWORD
        if (!password) return undefined
        const username = process.env.ZWEI_SERVER_USERNAME ?? "zwei"
