...
afterEach(async () => {
  delete process.env.ZWEI_CONFIG
  delete process.env.ZWEI_TUI_CONFIG
...
test("keeps server and tui plugin merge semantics aligned", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const local = path.join(dir, ".zwei")
...
test("loads tui config with the same precedence order as server config paths", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await fs.mkdir(path.join(dir, ".zwei"), { recursive: true })
      await Bun.write(
        path.join(dir, ".zwei", "tui.json"),
...
test("project config takes precedence over ZWEI_TUI_CONFIG (matches ZWEI_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.ZWEI_TUI_CONFIG = custom
    },
  })
...
test("ZWEI_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from ZWEI_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.ZWEI_CONFIG = path.join(customDir, "opencode.json")
    },
  })
...
test("loads .zwei/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".zwei"), { recursive: true })
      await Bun.write(path.join(dir, ".zwei", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
