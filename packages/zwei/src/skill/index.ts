...
export namespace Skill {
  ...
  const ZWEI_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
  ...
  const loadSkills = Effect.fnUntraced(function* (
    ...
    if (!Flag.ZWEI_DISABLE_EXTERNAL_SKILLS) {
    ...
    for (const dir of configDirs) {
      yield* scan(state, bus, dir, ZWEI_SKILL_PATTERN)
    }
  ...
  function trim(txt: string, max: number) {
    const one = txt.replace(/\s+/g, " ").trim()
    if (one.length <= max) return one
    return `${one.slice(0, Math.max(0, max - 1)).trimEnd()}…`
  }

  export function fmt(list: Info[], opts: { verbose: boolean; limit?: number; chars?: number }) {
    if (list.length === 0) return "No skills are currently available."
    const all = list.toSorted((a, b) => a.name.localeCompare(b.name))
    const max = opts.limit && opts.limit > 0 ? opts.limit : all.length
    ...
    if (opts.verbose) {
      return [
        ...
        ...out.flatMap((skill) => [
            ...
            `    <description>${opts.chars ? trim(skill.description, opts.chars) : skill.description}</description>`,
            `    <location>${pathToFileURL(skill.location).href}</location>`,
            "  </skill>",
          ]),
        ...(rest > 0 ? [`  <omitted>${rest}</omitted>`] : []),
        "</available_skills>",
      ].join("\n")
    }
    ...
    return [
      "## Available Skills",
      ...out.map((skill) => `- **${skill.name}**: ${trim(skill.description, opts.chars ?? 120)}`),
