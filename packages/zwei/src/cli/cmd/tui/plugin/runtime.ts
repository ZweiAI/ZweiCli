...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".zwei"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".zwei", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): Config.PluginOrigin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".zwei", "tui.json"),
  }
}
  ...
  async function load(api: Api) {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const records = Flag.ZWEI_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.ZWEI_PURE && config.plugin_origins?.length) {
