...
pub fn spawn_command(
    ...
    let mut envs = vec![
        (
            "ZWEI_EXPERIMENTAL_ICON_DISCOVERY".to_string(),
            "true".to_string(),
        ),
        (
            "ZWEI_EXPERIMENTAL_FILEWATCHER".to_string(),
            "true".to_string(),
        ),
        ("ZWEI_CLIENT".to_string(), "desktop".to_string()),
        (
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut env_prefix = vec![
                "ZWEI_EXPERIMENTAL_ICON_DISCOVERY=true".to_string(),
                "ZWEI_EXPERIMENTAL_FILEWATCHER=true".to_string(),
                "ZWEI_CLIENT=desktop".to_string(),
                "XDG_STATE_HOME=\"$HOME/.local/state\"".to_string(),
            ];
            env_prefix.extend(
                envs.iter()
                    .filter(|(key, _)| key != "ZWEI_EXPERIMENTAL_ICON_DISCOVERY")
                    .filter(|(key, _)| key != "ZWEI_EXPERIMENTAL_FILEWATCHER")
                    .filter(|(key, _)| key != "ZWEI_CLIENT")
...
pub fn serve(
    ...
    let envs = [
        ("ZWEI_SERVER_USERNAME", "opencode".to_string()),
        ("ZWEI_SERVER_PASSWORD", password.to_string()),
    ];
...
mod tests {
    ...
    fn merge_shell_env_keeps_explicit_overrides() {
        ...
        assert_eq!(merged.get("ZWEI_CLIENT"), Some(&"desktop".to_string()));
    }
