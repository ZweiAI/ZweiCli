...
describe("Session", () => {
  ...
  test("clear deletes every message but keeps the session row", async () => {
    await Instance.provide({
      directory: projectRoot,
      fn: async () => {
        const session = await Session.create({ title: "clear-test" })
        ...
        const removed = await Session.clear(session.id)
        expect(removed).toBe(3)

        const after = await Session.messages({ sessionID: session.id })
        expect(after.length).toBe(0)

        const still = await Session.get(session.id)
