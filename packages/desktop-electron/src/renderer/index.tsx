...
import "./styles.css"
import { useTheme } from "@zwei/ui/theme"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__ZWEI__ ??= {}
  const pending = window.__ZWEI__.deepLinks ?? []
  window.__ZWEI__.deepLinks = [...pending, ...urls]
  window.dispatchEvent(new CustomEvent(deepLinkEvent, { detail: { urls } }))
}

const listenForDeepLinks = () => {
  const startUrls = window.__ZWEI__?.deepLinks ?? []
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__ZWEI__?.wsl) return undefined
    return window.api.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__ZWEI__?.wsl) return result
  ...
  return {
    ...
    getWslEnabled: async () => {
      ...
      return window.__ZWEI__!.wsl ?? false
    },
