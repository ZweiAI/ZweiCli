  ...
  new Hono()
    ...
    .delete(
      ...
      describeRoute({
        ...
        description:
          "Delete every message (and its parts) from a session while keeping the session row. Primary consumer: the dual-agent TUI's `/clear` / `/clear1` / `/clear2` slash commands — the dual loop keeps the same child session ids across rounds, so we can't `delete` them wholesale.",
        operationId: "session.clear",
        responses: {
          200: {
            description: "Number of messages removed",
            content: {
              "application/json": {
                schema: resolver(z.object({ removed: z.number().int() })),
              },
            },
          },
          ...errors(400, 404),
      ...
      async (c) => {
        ...
        const removed = await Session.clear(params.sessionID)
