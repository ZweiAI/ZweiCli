...
By default, `bun dev` runs OpenCode in the `packages/zwei` directory. To run it against a different directory or repository:
...
```bash
./packages/zwei/script/build.ts --single
```

Then run it with:

```bash
./packages/zwei/dist/opencode-<platform>/bin/opencode
```

Replace `<platform>` with your platform (e.g., `darwin-arm64`, `linux-x64`).

- Core pieces:
  - `packages/zwei`: OpenCode core business logic & server.
  - `packages/zwei/src/cli/cmd/tui/`: The TUI code, written in SolidJS with [opentui](https://github.com/sst/opentui)
  - `packages/app`: The shared web UI components, written in SolidJS
  - `packages/desktop`: The native desktop app, built with Tauri (wraps `packages/app`)
  - `packages/plugin`: Source for `@zwei/plugin`
...
> [!NOTE]
> If you make changes to the API or SDK (e.g. `packages/zwei/src/server/server.ts`), run `./script/generate.ts` to regenerate the SDK and related files.
...
- If `spawn` does not work for you, you can debug the server separately:
  - Debug server: `bun run --inspect=ws://localhost:6499/ --cwd packages/zwei ./src/index.ts serve --port 4096`,
    then attach TUI with `opencode attach http://localhost:4096`
  - Debug TUI: `bun run --inspect=ws://localhost:6499/ --cwd packages/zwei --conditions=browser ./src/index.ts`
